...
import com.megazone.ERPSystem_phase3_LogisticsProduction.logistics.model.sales_management.QQuotation;
...
public class QuotationRepositoryImpl implements QuotationRepositoryCustom {
    ...
    @Override
    public List<Quotation> findBySearch(SearchDTO dto) {
    ...
    private BooleanBuilder buildSearchCondition(SearchDTO dto) {
        ...
        if (dto.getStartDate() != null && dto.getEndDate() != null) {
            condition.and(quotation.date.between(dto.getStartDate(), dto.getEndDate()));
        }

        // 거래처 코드 조건
        if (dto.getClientId() != null) {
            condition.and(quotation.quotationDetails.any().product.client.id.eq(dto.getClientId()));  // 거래처 코드 조건 추가
        }
        ...
        if (dto.getState() != null && !dto.getState().isEmpty()) {
            ...
            // QueryDSL Enum 값 비교
            condition.and(quotation.state.eq(enumState));
        }
